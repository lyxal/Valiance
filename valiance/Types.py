...
class Symbol:
    ...
    def __hash__(self) -> int:
        # Hash this type based on base and parts so that it can be stored in
        # dicts
    ...
    def __eq__(self, other: object):
        ...
        return self.base == other.base and all(
            p1 == p2 for p1, p2 in zip(self.parts, other.parts, strict=False)
        )
            ...
            if isinstance(parameter, OptionalTypeParameter) and isinstance(
                ...
                for c in mro_parameter:
                    if c in mro_last:
                        ...
                        break
                if lca is not None:
                    # Then, create a new parameter of the LCA type with the
                    # maximum rank of the two parameters
...
class NamedType(Type):
    ...
    def __eq__(self, other: object):
        if not isinstance(other, NamedType):
            return False
        # Do not compare tags or parameters for equality since they are not part
        # of the base type, but rather modifiers on the type. Only compare name
        # and generics.
...
class IntersectionType(Type):
    ...
    def flatten(self):
        # If an intersection contains other intersections, flatten them into a
        # single self.types list
        flattened_types = []
        for t in self.types:
