...
"""
        ...
        for output in outputs:
            ...
            self.assertEqual(
                value.value,
                [RuntimeNumber("5"), RuntimeNumber("6"), RuntimeNumber("7")],
            )
            self.assertEqual(
        ...
        for source in sources:
            with self.subTest(source=source):
                ...
                for output in outputs:
                    [value] = output
                    self.assertIsInstance(value, TaggedValue)
                    self.assertEqual(value.value, RuntimeNumber("3"))
    ...
    def test_constructed_flow_is_tag_name_and_fixed_arity_agnostic(self):
        ...
        for tag_name in tag_names:
            for arity in arities:
                ...
                params = ", ".join(f"value{index}: Number" for index in range(arity))
                body = "$value0"
...
"""
        ...
        for output in outputs:
            [value] = output
            self.assertIsInstance(value, TaggedValue)
            self.assertEqual(value.value, RuntimeNumber("1"))
            self.assertEqual({tag.name for tag in value.tags}, {"sticky"})
...
"""
        ...
        for output in outputs:
            self.assertEqual(output, [RuntimeNumber("1")])
...
"""
        ...
        for executable in (program, loads(dumps(program))):
            [value] = run(executable)
            self.assertEqual(value, RuntimeNumber("1"))
