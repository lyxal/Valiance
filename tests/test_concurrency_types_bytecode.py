...
from valiance.vtypes import (
    Integer, Number, String, Task, TaskType, V, _solve, _substitute,
...
class TaskTypeTests(unittest.TestCase):
    def test_task_preserves_native_output_row(self):
        task = Task(Integer, String)
        self.assertIsInstance(task, TaskType)
        self.assertEqual(task.outputs, (Integer, String))
        self.assertEqual(show(task), "Task[Integer, String]")
    ...
    def test_task_output_generics_substitute_positionally(self):
        ...
        self.assertEqual(_substitute(task, {"T": Integer}), Task(Integer, String))

    def test_task_rows_are_invariant(self):
        self.assertTrue(assignable(Task(Integer), Task(Integer)))
        self.assertFalse(assignable(Task(Integer), Task(Number)))
        self.assertFalse(assignable(Task(Integer), Task(Integer, String)))

    def test_task_row_participates_in_generic_solving(self):
        self.assertEqual(_solve(Task(V("T")), Task(Integer)), {"T": [Integer]})

    def test_task_effects_are_preserved_and_invariant(self):
        from valiance.vtypes.symbols import Symbol
        io = ElementTag(Symbol("io"))
        task = Task(Integer, effects=(io,))
        self.assertEqual(task.effects, frozenset({io}))
        self.assertFalse(assignable(task, Task(Integer)))
    ...
    def test_channel_nominal_argument_is_invariant(self):
        ...
        context = Context()
        self.assertFalse(assignable(N(Symbol("Channel"), Integer), N(Symbol("Channel"), Number), context))
...
class ConcurrencyBytecodeTests(unittest.TestCase):
    ...
    def test_surface_task_annotation_normalizes_to_task_output_row(self):
        ...
        definition = parse(
            "define f -> Task[Integer, String] => "
            "fn -> Integer, String => 1 \"one\" end | spawn end"
        )[0]
        declared = definition.function.returns[0]
        normalized = normalize(declared)
        self.assertIsInstance(normalized, TaskType)
        self.assertEqual(normalized.outputs, (Integer, String))
