...
class OptionalMemberParserTests(unittest.TestCase):
    ...
    def test_parses_optional_safe_assignment(self):
        ...
        self.assertTrue(
            any(isinstance(node, FieldSetNode) and node.optional_safe for node in nodes)
        )
    ...
    def test_parses_mixed_member_chain(self):
        ...
        named_accesses = [node for node in named if isinstance(node, FieldAccessNode)]
        stack_accesses = [node for node in stack if isinstance(node, FieldAccessNode)]
        self.assertEqual(
