...
class ConcurrencyExecutionTests(unittest.TestCase):
    def test_spawn_wait_executes_once(self):
        self.assertEqual(
            execute("fn -> Integer => 42 end | spawn | wait"),
            [RuntimeNumber(42)],
        )
    ...
    def test_spawn_captures_explicit_argument_at_spawn(self):
        self.assertEqual(
            execute(
                "10 fn (value: Integer) -> Integer => $value 2 * end | spawn | wait"
            ),
            [RuntimeNumber(20)],
        )
    ...
    def test_wait_restores_multiple_outputs(self):
        self.assertEqual(
            execute('fn -> Integer, String => 1 "one" end | spawn | wait'),
            [RuntimeNumber(1), "one"],
        )

    def test_concurrent_scope_preserves_body_outputs(self):
        self.assertEqual(
            execute("concurrent -> Integer => 7 end"),
            [RuntimeNumber(7)],
        )
    ...
    def test_discarded_child_is_joined_at_scope_end(self):
        self.assertEqual(
            execute(
                """concurrent -> Integer =>
  $ignored = fn -> Integer => 3 end | spawn
  9
end"""
            ),
            [RuntimeNumber(9)],
...
$task wait"""
    ...
    def test_root_scope_joins_unobserved_child(self):
        self.assertEqual(
            execute("$task = fn -> Integer => 8 end | spawn\n1"),
            [RuntimeNumber(1)],
        )

    def test_concurrency_bytecode_executes_after_roundtrip(self):
        analyser = Analyser()
        typed = analyser.analyse(parse("fn -> Integer => 5 end | spawn | wait"))
...
$base"""
    ...
    def test_parent_write_after_spawn_does_not_change_child_argument(self):
        source = """$base = [1, 2, 3]
$operation = fn (items: Integer+) -> Integer+ => $items end
...
$base"""
    ...
    def test_child_write_does_not_change_parent_argument(self):
        source = """$base = [1, 2, 3]
$operation = fn (items: Integer+) -> Integer+ =>
  $local = $items
  $local[1] := 8
  $local
end
...
$operation = fn (items: Integer+) -> Integer => $items first end
...
$task wait"""
    ...
    def test_discarded_failed_child_is_not_ignored(self):
        source = """concurrent -> Integer =>
  $ignored = fn -> Integer => RuntimeFault("child failed") panic end | spawn
  9
end"""
    ...
    def test_vectorised_wait_failure_is_selected_by_input_order(self):
        source = """$tasks = [
  fn -> Integer => RuntimeFault("first failure") panic end | spawn,
  fn -> Integer => RuntimeFault("second failure") panic end | spawn
]
$tasks wait"""
    ...
    def test_body_panic_cancels_and_joins_spawned_child(self):
        source = """concurrent -> Integer =>
  $pending = fn -> Integer => 1 end | spawn
  RuntimeFault("body failed") panic
    ...
    def test_handle_returned_from_closed_scope_is_terminal_and_waitable(self):
        source = """$task = concurrent -> Task[Integer] =>
  fn -> Integer => 12 end | spawn
end
$task wait
$task wait"""
