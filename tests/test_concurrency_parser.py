...
from valiance.vtypes import Integer, String
...
class ConcurrentParserTests(unittest.TestCase):
    def test_explicit_parameters_and_multiple_returns(self):
        nodes = parse(
            """10 20
concurrent (a: Integer, b: Integer) -> Integer, String =>
  $a
  $b
end"""
        )
        ...
        self.assertEqual(block.returns, (Integer, String))
    ...
    def test_missing_arrow_is_rejected(self):
        with self.assertRaises(ParseError):
            parse("concurrent (x: Integer) x end")

    def test_pretty_print_includes_scope_contract(self):
        rendered = pretty_ast(parse("concurrent () -> Integer => 1 end"))
